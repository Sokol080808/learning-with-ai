...
## Статус треков (на 2026-06-23)

- **Все 5 треков на баре:** Python, C++, Rust, Deep Learning, CAOS — эталоны на `reference`,
  `reference-green` зелёный, property-тесты кусают, теория seminar-глубины, существенная задача
  в каждом модуле, courses-ci проверяет скелет.
- **Все 4 source-grounded ре-аудита против канона завершены:** C++ (PR #24), Python (#25),
  Rust (#27 — добавлены модули конкуренции и `async`), Deep Learning (#28 — аугментация/transfer/
  метрики фолд-ин + nanoGPT-сэмплирование). Курсы доведены; дальше — точечные улучшения по
  обратной связи учеников.
...
**Сверено / СДЕЛАНО (PR #27):** добавлены модуль **15-concurrency** (потоки/каналы/`Arc<Mutex>`/`Send`+`Sync`,
The Rust Book гл.16) и **16-async** (`async`/await на крейте `futures`); теория углублена во всех модулях.
...
**Сверено / СДЕЛАНО (PR #28):** аугментация (модуль 13), transfer learning (17), метрики/оценка (10),
позиционное кодирование (15), nanoGPT-сэмплирование `temperature`/`top-k` (капстоун) — всё фолд-ин, без
новых модулей; теория углублена во всех 18. (Трек был самый полный — это была шлифовка, не пробелы.)
