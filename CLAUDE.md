...
- Regenerate the graph only when **course material** changes, and only on the `main` branch: `graphify update .` (scans the whole workspace — venv/target/build/graphify-out are auto-excluded; AST-only, no API cost). Do **not** regenerate on any branch other than `main` (e.g. `solutions` or `reference`) — it would diverge `graph.json` from `main` and cause merge conflicts. Those branches inherit the graph from `main` and refresh it via `git merge main`. (Graph regeneration is now **manual** — the auto-update Stop hook was removed; after changing course material on `main`, run `graphify update .` yourself. `.claude/settings.json` keeps only the dialogue-capture Stop hook.)
