...
def replace_head(model: nn.Sequential, new_out: int) -> nn.Module:
    ...
    Пример:
        ...
        # model(x).shape[-1] == 3
    """
    # индекс последнего Linear в Sequential
    last_linear_idx = None
    for idx, submodule in enumerate(model):
        if isinstance(submodule, nn.Linear):
            last_linear_idx = idx
    if last_linear_idx is None:
        raise ValueError("replace_head: в модели нет ни одного nn.Linear")

    old = model[last_linear_idx]
    model[last_linear_idx] = nn.Linear(old.in_features, new_out)
    return model
...
def transfer_finetune(
    ...
    """
    # Архитектура каркаса должна совпадать с донором, чтобы load_model влил веса.
    skeleton = _build_backbone_skeleton()
    load_model(skeleton, backbone_path)

    # Снимок весов тела (всё, кроме последнего модуля) — для проверки неизменности.
    backbone_modules = list(skeleton)[:-1]
    backbone_before = [
        p.detach().clone()
    ...
    # Свежая голова под новую задачу + заморозка тела.
    replace_head(skeleton, new_out)
    freeze_backbone(skeleton, n_keep_trainable_tail=1)

    trainable_params = int(
        sum(p.numel() for p in skeleton.parameters() if p.requires_grad)
    )
    frozen_params = int(
        sum(p.numel() for p in skeleton.parameters() if not p.requires_grad)
    )

    # Обучаем ТОЛЬКО обучаемые параметры (голову).
    trainable = [p for p in skeleton.parameters() if p.requires_grad]
    optimizer = optim.SGD(trainable, lr=lr)
    criterion = nn.CrossEntropyLoss()

    set_mode(skeleton, True)
    with torch.no_grad():
        loss_start = criterion(skeleton(x), y).item()
    ...
    # Веса тела не должны были сдвинуться ни на бит.
    backbone_after = [
        p.detach()
        for m in list(skeleton)[:-1]
        for p in m.parameters()
    ]
    backbone_unchanged = all(
        torch.equal(b, a) for b, a in zip(backbone_before, backbone_after)
    )
    ...
    return {
        ...
        "loss_decreased": bool(loss_end < loss_start),
    }
