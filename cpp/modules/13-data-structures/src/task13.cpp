...
// Подсказка по структуре: раздели вектор пополам, рекурсивно отсортируй каждую
// половину, затем слей (merge) две отсортированных части в один.

std::vector<int> merge_sort(std::vector<int> v) {
    // TODO: реализуй merge sort.
    // Шаги: если v.size() <= 1 — сразу return v (базовый случай).
    // Иначе: разрежь на [0, mid) и [mid, end), рекурсивно отсортируй, слей.
    return v; // неверный стаб: возвращает НЕСОРТИРОВАННЫЙ вектор
}
